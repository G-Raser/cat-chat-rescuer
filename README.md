# CatChat Rescuer v0.5.5

CatChat Rescuer 是一个用于归档 ChatGPT 对话、保存已展示思考轨迹，并在必要时抢救超长对话的 Edge / Chrome 扩展。

当前实验分支采用 **API-first** 架构：优先直接读取当前 ChatGPT 会话可访问的 conversation JSON；旧版 DOM / 增量扫描工具仍保留为救援模式。
...
v0.5.5 起，思考导出的时间戳作为回合元数据放在**最底部**：正文之后依次显示可用的 `Worked for ...`、`model`，最后才是 `time`，避免时间戳卡在标题与正文之间打断阅读。
...
v0.5.2 在现代 UI 前增加了 `boot-hide.js`。

旧 DOM 核心仍负责创建基础面板，但基础旧 UI 在现代三块界面完整挂载之前不会显示，因此刷新页面时不应再先闪一下旧版按钮面板。
...
v0.5.4 继续统一次级 UI 排版，把 `开发诊断 / 完整原始轨迹` 的折叠标题收回与其他次级控件一致的字号、行高和字重，避免突然出现一个视觉上过大的标题。

### 面板没有出现时

v0.5.1 起支持恢复 / 唤醒入口：如果重新加载扩展后，某个已打开的 ChatGPT 标签页没有成功出现面板，可以在该标签页点击一次扩展图标。

v0.5.2 的恢复路径同样先运行 `boot-hide.js`，避免恢复过程中再次闪出旧 UI。
...
旧实验脚本可能仍留在分支历史中，但 v0.5.5 manifest 不再把它们放进默认运行链。
