(() => {
  ...
  function mount() {
    ...
    try {
      ...
      quick.innerHTML = `<div class="ccr-section-heading"><div><strong>读取内容</strong><small>API 直读一次，后续导出共用缓存</small></div><span class="ccr-badge">${VERSION}</span></div><input id="ccr-api-reference" type="text" placeholder="留空读取当前对话；或粘贴 /c/… 链接 / conversation ID"><button id="ccr-api-read" class="ccr-primary" type="button">读取内容</button><div class="ccr-export-row"><button id="ccr-api-md" disabled>可读 MD</button><button id="ccr-api-raw" disabled>Raw JSON</button></div><div class="ccr-export-options"><label class="ccr-check"><input id="ccr-conversation-timestamps" type="checkbox"><span>导出时间戳</span></label></div><div id="ccr-api-status" class="ccr-api-status">尚未读取</div><details class="ccr-label-settings"><summary>导出称呼</summary><div class="ccr-label-grid"><label><span>用户</span><input id="ccr-label-user" maxlength="40"></label><label><span>助手</span><input id="ccr-label-assistant" maxlength="40"></label><button id="ccr-label-reset" type="button">恢复 User / Assistant</button></div></details>`;
      ...
      const title = q(".ccr-title span", panel); if (title) { title.textContent = "🐾 猫茶抢救器 · 实验"; title.onclick = () => collapse(panel, !panel.classList.contains("ccr-collapsed")); }
