async function readConversationFromPage(conversationId, projectId = null) {
  ...
  for (const headers of headerCandidates) {
    try {
      ...
      conversation.catchat_api_source = {
  ...
  for (const headers of headerCandidates) {
    try {
      return await readPaginated(headers);
    } catch (error) {
      lastError = error;
    }
  }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (!tabId || !message.conversationId) {
    sendResponse({ ok: false, error: "无法识别来源标签或 conversation ID" });
    return false;
  }
  chrome.scripting.executeScript({
    target: { tabId },
    world: "MAIN",
    func: readConversationFromPage,
    args: [message.conversationId, message.projectId || null]
  }).then((results) => {
    const conversation = results?.[0]?.result;
    if (!conversation) throw new Error("页面没有返回 conversation JSON");
...
chrome.action.onClicked.addListener(async (tab) => {
  ...
  try {
    ...
    await chrome.scripting.executeScript({ target: { tabId: tab.id }, files: ["content.js", "api-experiment.js"] });
  } catch (error) {
